...
def _last_voice_decision() -> str:
    for entry in reversed(app_state.get_audit_log()):
        if entry.action == "confirm_suggestion":
            return "Confirmed"
        if entry.action == "dismiss_suggestion":
            return "Dismissed"
...
def voice_hud() -> None:
    ...
    st.markdown(
        ...
        '<div class="aegis-subtle">Command capture and recent medic action state.</div></div>',
    ...
    st.markdown(
        (
            "<div class='aegis-row'>"
            f"<div class='aegis-list-title'>Last action</div>"
